...
!DEC$ ATTRIBUTES DLLEXPORT, ALIAS:'DISCON' :: DISCON
   ...
   ! the NREL Offshore 5MW baseline wind turbine.  This routine is a modified 
   ! version of the routine written by J. Jonkman of NREL/NWTC for use in the 
   ! IEA Annex XXIII OC3 studies. This routine derates the turbine at t = 150 
   ! seconds.
...
CHARACTER(  25), PARAMETER   :: FmtDat    = "(F8.3,99('"//Tab//"',ES10.3E2,:))" ! The format of the debugging data
	...
	!Local variables used for derating
REAL(4)						 :: DR = 0.2								!The magnitude of the derate command (0 to 1)	
REAL(4)                      :: t_DRstart = 150.0 								! Time when the derate command will be issued.
REAL(4)                      :: t_DRend = 250.0 								! Time when the end derate command will be issued. 
REAL(4)						 :: pDR = 0.2								!- poles of the second order derate input filter.
REAL(4)                      :: FF_pwrFactor = 1.0 								! The derate factor. A fraction of 1, where 1 is not derated.
REAL(4)                      :: FF_MinPit = 0.0 								! increase in minimum pitch dictated by derate scheme.
REAL(4)						 :: PC_MinPitDR     = PC_MinPit                     ! Minimum pitch setting dictated by derate scheme, rad.
REAL(4)           			 :: PC_RefSpdDR     =    PC_RefSpd                  ! Desired (reference) HSS speed adjusted for derating, rad/s.
REAL(4)           			 :: VS_Rgn2KDR      =    VS_Rgn2K                  ! Generator torque constant in Region 2 (HSS side) adjusted for derating, N-m/(rad/s)^2.
REAL(4)           			 :: VS_Rgn3MPDR     =    VS_Rgn3MP                ! Minimum pitch angle at which the torque is computed as if we are in region 3 regardless of the generator speed, adjusted for derating, rad. -
REAL(4)           			 :: VS_RtGnSpDR     =     VS_RtGnSp                    ! Rated generator speed (HSS side) adjusted for derating, rad/s.
REAL(4)			           	 :: VS_RtPwrDR      = VS_RtPwr                       ! Rated generator generator power in Region 3 adjusted for derating, Watts. 
REAL(4), SAVE                :: VS_Slope15DR                                      ! Torque/speed slope of region 1 1/2 cut-in torque ramp adjusted for derating, N-m/(rad/s).
REAL(4), SAVE                :: VS_Slope25DR                                      ! Torque/speed slope of region 2 1/2 induction generatoradjusted for derating, N-m/(rad/s).
REAL(4), SAVE                :: VS_TrGnSpDR                                       ! Transitional generator speed (HSS side) between regions 2 and 2 1/2, adjusted for derating, rad/s.
REAL(4), SAVE                :: VS_SySpDR                                         ! Synchronous speed of region 2 1/2 induction generator adjusted for derating, rad/s.
...
IF ( iStatus == 0 )  THEN  ! .TRUE. if were on the first call to the DLL
   ...
   cMessage = 'Running with torque and pitch control of the NREL offshore '// &
              '5MW baseline wind turbine from DISCON.dll. Controller derates'// &
			  'the turbine by 20% at t = 150 s. This version of the NREL'// &
			  '5MW controller written by Eric Anderson on 10/7/2016.'
...
!=======================================================================
   ...
   IF ( ( Time*OnePlusEps - LastTimeVS ) >= VS_DT )  THEN
		...
		IF (Time >= t_DRend) THEN
			!return turbine to normal operation
			FF_pwrFactor = 1.0 - DR + DR*(1.0 - pDR*(Time - t_DRend)*EXP(-pDR*(Time - t_DRend)) - EXP(-pDR*(Time - t_DRend)))
		ELSEIF (Time >= t_DRstart) THEN
			!Derate turbine
			FF_pwrFactor = 1.0 - DR*(1.0 - pDR*(Time - t_DRstart)*EXP(-pDR*(Time - t_DRstart)) - EXP(-pDR*(Time - t_DRstart)))
		ENDIF
		
		!Adjust torque control parameters based for derating
		VS_Rgn2KDR = VS_Rgn2K/(FF_pwrFactor**2) 	! Region 2 torque constant
		VS_RtGnSpDR = VS_RtGnSp*FF_pwrFactor	! Rated generator speed
		VS_RtPwrDR = VS_RtPwr*FF_pwrFactor		! Rated power
		VS_Rgn3MPDR = VS_Rgn3MP + FF_MinPit		! Minimum pitch to mandate region 3 control
		VS_Slope15DR = VS_Rgn2Sp*(VS_Rgn2Sp**2)*VS_Rgn2KDR/(VS_Rgn2Sp-VS_CtInSp)	! Slope in region 1.5	
		VS_Slope25DR = (VS_RtPwrDR/VS_RtGnSpDR)/(VS_RtGnSpDR - (VS_RtGnSpDR/1.1)) ! Slope in region 2.5
		VS_SySpDR = VS_RtGnSpDR/1.1				! Speed where the turbine operates as a synchronous induction generator
		VS_TrGnSpDR = (VS_Slope25DR - SQRT(VS_Slope25DR**2 - 4*VS_Rgn2KDR*VS_SySpDR*VS_Slope25DR))/(2*VS_Rgn2KDR) !Transition speed from region 2 to region 2.5
			
			
   ! Compute the generator torque, which depends on which region we are in:

      IF ( (   GenSpeedF >= VS_RtGnSpDR ) .OR. (  PitCom(1) >= VS_Rgn3MPDR ) )  THEN ! We are in region 3 - power is constant
         GenTrq = VS_RtPwrDR/GenSpeedF
      ...
      ELSEIF ( GenSpeedF <  VS_Rgn2Sp )  THEN                                    ! We are in region 1 1/2 - linear ramp in torque from zero to optimal
         GenTrq = VS_Slope15DR*( GenSpeedF - VS_CtInSp )
      ELSEIF ( GenSpeedF <  VS_TrGnSpDR )  THEN                                    ! We are in region 2 - optimal torque is proportional to the square of the generator speed
         GenTrq = VS_Rgn2KDR*GenSpeedF*GenSpeedF
      ELSE                                                                       ! We are in region 2 1/2 - simple induction generator transition region
         GenTrq = VS_Slope25DR*( GenSpeedF - VS_SySpDR   )
      ENDIF
...
!=======================================================================
   ...
   !   commanded pitch angle for blade 1:
      ...
      GK = 1.0/( 1.0 + PitCom(1)/PC_KK )
		...
		! Adjust pitch control parameters based on derate factor
		
		PC_RefSpdDR = PC_RefSpd*FF_pwrFactor
		PC_MinPitDR = PC_MinPit + FF_MinPit
		
   ! Compute the current speed error and its integral w.r.t. time; saturate the
   !   integral term using the pitch angle limits:

      SpdErr    = GenSpeedF - PC_RefSpdDR                                 ! Current speed error
